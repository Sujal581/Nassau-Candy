...
with cl:
    ...
    chart_note("Peaks indicate high-demand periods that may strain logistics capacity. Use this trend to plan carrier resources and inventory buffers ahead of seasonal surges.")
...
with cr:
    ...
    chart_note("A healthy operation targets ≥70% on-time. Adjust the delay threshold slider in the sidebar to redefine what counts as delayed for your SLA standards.")
...
with cl2:
    ...
    chart_note("A tight, left-skewed histogram means consistently fast fulfillment. A long right tail signals outlier orders inflating the average — investigate the root cause of those shipments.")
...
with cr2:
    ...
    if "Route" in df.columns:
        ...
        chart_note("High-volume routes benefit most from dedicated carrier lanes and rate negotiations. Low-volume routes may be candidates for consolidation to reduce overhead.")
...
with cl3:
    ...
    chart_note("Bar height = avg lead time (lower is faster). Bar color = delay rate — a fast mode with a warm color is quick on average but inconsistent. Focus improvement efforts there.")
...
state_df = df.groupby("State/Province", observed=True).agg(
...
chart_note("States sorted from fastest to slowest avg lead time. Red-highlighted cells flag the states most in need of route redesign or carrier renegotiation. Use the sidebar to filter to a specific state.")
