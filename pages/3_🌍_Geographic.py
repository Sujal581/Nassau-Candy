...
from style import (inject_css, sidebar_brand, page_header, section_header,
                   kpi_card, chart_label, chart_note, insight_card, apply_plot_layout, footer, COLORS)
...
fig_map = go.Figure(go.Heatmap(
    ...
    colorscale=[[0, "#00e5ff"], [0.5, COLORS["amber"]], [1, COLORS["red"]]],
    showscale=True,
    colorbar=dict(
...
chart_note("Color intensity represents avg lead time per state — cyan = fast delivery, amber/red = slow. Grey cells have no data in the current filter window. Hover any state tile for exact lead time and shipment count.")
...
with cl:
    ...
    chart_note("States sorted fastest to slowest. A cluster of red bars in a specific region likely points to a shared carrier or distribution center issue rather than isolated state-level problems.")
...
with cr:
    ...
    chart_note("Large bubbles in the upper area (high volume + high lead time) deserve the most attention — improving those states moves the overall average the most.")
...
chart_note("Top 15 states by total shipment count. High-volume states have the greatest leverage on overall KPIs — even a 1-day lead time improvement here creates more impact than a 3-day improvement in a low-volume state.")
...
chart_note("Tier thresholds: Excellent ≤3 days, Good ≤5, Slow ≤7, Critical >7. States in the Critical tier should be escalated for immediate carrier review and possible route redesign.")
st.download_button(
...
if len(critical):
    insight_card(f"🔴 <strong>{len(critical)}</strong> state(s) in Critical tier (>7 days): <strong>{', '.join(critical['State'].tolist())}</strong>.", "error")
