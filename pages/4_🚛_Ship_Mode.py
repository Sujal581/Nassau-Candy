...
with col1:
    ...
    chart_note("Rank labels show each mode's speed order (#1 = fastest). Bar color reflects avg lead time — a short bar with a warm color indicates a mode that is fast on average but still accumulating delays.")
...
with col2:
    ...
    chart_note("Heavy concentration in one mode creates single-carrier dependency risk. A more balanced split improves resilience to service disruptions and gives leverage in rate negotiations.")
...
chart_note("Modes above 30% delay rate warrant immediate carrier renegotiation or substitution for time-sensitive orders. Adjust the delay threshold in the sidebar to see how each mode's rate changes.")
...
trend_df = trend.groupby(["Month", "Ship Mode"], observed=True)["Lead Time"].mean().reset_index()
...
chart_note("Diverging lines between modes over time indicate one mode is degrading relative to others. A rising trend across all modes simultaneously may signal a systemic network issue rather than a carrier-specific one.")
...
chart_note("Composite score = 70% speed weight + 30% volume weight. A high-efficiency mode is both fast and heavily utilized — the best overall value. Low-score modes may cost more per day of lead time than alternatives.")
...
chart_note("Full comparison table across all ship modes. Sort by any column to identify trade-offs — a mode might rank well on speed but poorly on delay rate, revealing hidden inconsistency.")
