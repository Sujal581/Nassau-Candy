...
from style import (inject_css, sidebar_brand, page_header, section_header,
                   kpi_card, chart_label, chart_note, insight_card, apply_plot_layout,
...
div_df = (
    df.groupby("Division", observed=True)
    .agg(
...
with cl:
    ...
    chart_note("Divisions with higher bars may face capacity constraints, scheduling backlogs, or longer pick-and-pack cycles. Compare against historical data to identify whether performance is improving or degrading.")
...
with cr:
    ...
    chart_note("Unequal distribution may signal underutilization of some divisions. Rebalancing order allocation can reduce lead times across the network by spreading peak-load pressure.")
...
with cl2:
    ...
    chart_note("High delay rates in a division often trace to specific carrier assignments or geographic coverage gaps. Start route-level investigations for these divisions to pinpoint the exact cause.")
...
with cr2:
    ...
    chart_note("Divisions in the top-right (high lead + high delay) carry the most operational risk. Bubble size reflects their volume impact on the network — large bubbles here demand urgent attention.")
...
monthly_div = (
    df_copy.groupby([pd.Grouper(key="Order Date", freq="ME"), "Division"], observed=True)["Lead Time"]
    .mean()
    .reset_index()
)
...
chart_note("Upward trends in a division's lead time are an early warning signal — catching degradation here prevents it from surfacing as a KPI problem weeks later. Month-by-month drops after a peak often indicate a corrective action working.")
...
chart_note("Divisions serving more states are critical to nationwide coverage. Disruptions or delays in high-reach divisions have a wider geographic blast radius and affect more customers simultaneously.")
...
chart_note("Risk Score = 50% avg lead time + 50% delay rate. Tier thresholds: Low <25, Medium <50, High <75, Critical ≥75. Use the Risk Tier column to quickly identify which divisions need executive attention.")
st.download_button(
